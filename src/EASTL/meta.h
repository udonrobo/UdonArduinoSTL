...
namespace std
{
	namespace meta
	{
		...
		namespace Internal
		{
			...
			{
				static EA_CONSTEXPR_OR_CONST int value = is_same<T, Head>::value ? I : get_type_index<I + 1, T, Types...>::value;
			};
		...
		{
			static EA_CONSTEXPR_OR_CONST int value = (is_same<T, H>::value ? 1 : 0) + type_count<T, Types...>::value;
		};
		...
		struct unique_type_list : public unique_type_list<Ts...>
		{
			...
			static enable_if_t<!disjunction<is_same<T, Args>...>::value, type_list<T, Args...>>
			types(type_list<Args...>);

			template <typename... Args>
			static enable_if_t<disjunction<is_same<T, Args>...>::value, type_list<Args...>>
			types(type_list<Args...>);
