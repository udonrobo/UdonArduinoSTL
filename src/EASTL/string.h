...
namespace std
{
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::append_sprintf_va_list(const value_type* pFormat, va_list arguments)
	{
		...
		if (nReturnValue > 0)
		{
			...
			nReturnValue = std::Vsnprintf(internalLayout().BeginPtr() + nInitialSize, static_cast<size_t>(nReturnValue) + 1, pFormat, arguments);
		}
	...
	basic_string<T, Allocator>::GetNewCapacity(size_type currentCapacity, size_type minimumGrowSize)
	{
		...
		const size_type nNewCapacity = std::max_alt(currentCapacity + minimumGrowSize, currentCapacity * 2);
