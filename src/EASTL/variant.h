...
namespace eastl
{
	...
	// 20.7.4, value access
	//
	template <class T, class... Types, ssize_t I = meta::get_type_index<T, Types...>::value>
	...
	class variant
	{
		...
		static_assert(disjunction<is_void<Types>...>::value == false,      "variant does not allow void as an alternative type");
		static_assert(disjunction<is_reference<Types>...>::value == false, "variant does not allow references as an alternative type");
		static_assert(disjunction<is_array<Types>...>::value == false,     "variant does not allow arrays as an alternative type");

		using variant_index_t = size_t;
		using variant_storage_t = variant_storage<conjunction<is_trivially_destructible<Types>...>::value, Types...>;
	...
	public:
		...
		template <typename TT0 = T_0, typename = enable_if_t<is_default_constructible<TT0>::value>>
		...
		template <bool enable = conjunction<is_copy_constructible<Types>...>::value,
		...
		template <bool enable = conjunction<is_move_constructible<Types>...>::value, typename = enable_if_t<enable>> // add a dependent type to enable sfinae
		EA_CONSTEXPR variant(variant&& other) EA_NOEXCEPT(conjunction<is_move_constructible<Types>...>::value)
		...
		template <typename T,
		          ...
		          typename = enable_if_t<!is_same<decay_t<T>::value, variant>>,
		          size_t I = meta::get_type_index<decay_t<T_j>, Types...>::value>
		EA_CONSTEXPR variant(T&& t) EA_NOEXCEPT(is_nothrow_constructible<T_j, T>::value)
		   : mIndex(variant_npos), mStorage()
		{
		...
		template <size_t I,
		          class... Args,
		          class = enable_if_t<conjunction<integral_constant<bool, (I < sizeof...(Types))>,
		                                            is_constructible<meta::get_type_at_t<I, Types...>, Args...>>>::value>
		...
		template <
		    class T,
		    class... Args,
		    size_t I = meta::get_type_index<T, Types...>::value,
		...
		template <class T,
		          ...
		          class... Args,
		          size_t I = meta::get_type_index<T, Types...>::value,
		          typename = enable_if_t<conjunction<is_constructible<T, std::initializer_list<U>&, Args...>,
		...
		template <size_t I,
		          ...
		          typename =
		              enable_if_t<conjunction<is_constructible<T, Args...>, meta::duplicate_type_check<T, Types...>>::value>>
		...
		template <size_t I,
		          ...
		          class... Args,
		          typename T = meta::get_type_at_t<I, Types...>,
		          typename = enable_if_t<conjunction<is_constructible<T, std::initializer_list<U>&, Args...>, meta::duplicate_type_check<T, Types...>>::value>>
		...
		{
			static_assert(I >= 0, "T not found in type-list.");
			static_assert((meta::type_count<T_j, Types...>::value == 1),
		...
		template <bool enable = conjunction<conjunction<is_copy_constructible<Types>...>,
		                                      conjunction<is_copy_assignable<Types>...>::value>,
		          typename = enable_if_t<enable>> // add a dependent type to enable sfinae
		...
		template <bool enable = conjunction<conjunction<is_move_constructible<Types>...>,
		                                      conjunction<is_move_assignable<Types>...>::value>,
		          typename = enable_if_t<enable>> // add a dependent type to enable sfinae
		variant& operator=(variant&& other)
		    EA_NOEXCEPT(conjunction<conjunction<is_nothrow_move_constructible<Types>...>,
		                              conjunction<is_nothrow_move_assignable<Types>...>>::value)
		{
			if (this != &other)
			{
