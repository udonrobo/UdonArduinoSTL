...
namespace std
{
	...
	public:
		...
		unique_ptr(pointer pValue, typename std::conditional<std::is_lvalue_reference<deleter_type>::value, deleter_type, typename std::add_lvalue_reference<const deleter_type>::type>::type deleter) EA_NOEXCEPT
	...
	public:
		...
		template <typename P>
		unique_ptr(P pArray, typename std::conditional<std::is_lvalue_reference<deleter_type>::value, deleter_type,
		...
		template <typename P>
		unique_ptr(P pArray, typename std::remove_reference<deleter_type>::type&& deleter, std::enable_if_t<Internal::is_array_cv_convertible<P, pointer>::value>* = 0) EA_NOEXCEPT
		...
		unique_ptr(unique_ptr<U, E>&& u, typename enable_if<Internal::is_safe_array_conversion<T, pointer, U, typename unique_ptr<U, E>::pointer>::value &&
														   ...
														   (!std::is_lvalue_reference<deleter_type>::value || std::is_same<E, deleter_type>::value)>::type* = 0) EA_NOEXCEPT
			: mPair(u.release(), std::forward<E>(u.get_deleter())) {}
	...
	template <typename T>
	inline typename std::enable_if<std::is_unbounded_array<T>::value, std::unique_ptr<T>>::type make_unique(size_t n)
	{
	...
	// It's not possible to create a unique_ptr for arrays of a known bound (e.g. int[4] as opposed to int[]).
	template <typename T, typename... Args>
	typename std::enable_if<std::is_bounded_array<T>::value>::type
