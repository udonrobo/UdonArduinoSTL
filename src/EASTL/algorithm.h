...
namespace std
{
	...
	ForwardIterator max_element(ForwardIterator first, ForwardIterator last, Compare compare)
	{
		...
		template <typename T>
		inline EA_CONSTEXPR typename std::enable_if<!std::is_scalar<T>::value, const T&>::type
	...
	*/
	...
	{
		std::set_difference_2(first1, last1, first2, last2, result1, result2, std::less<>{});
	}
	...
	{
		return std::set_decomposition(first1, last1, first2, last2, result1, result2, result3, std::less<>{});
	}
	...
	bool next_permutation(BidirectionalIterator first, BidirectionalIterator last)
	{
		...
		return std::next_permutation(first, last, std::less<value_type>());
	}
	...
	{
		return std::clamp(v, lo, hi, std::less<>());
	}
