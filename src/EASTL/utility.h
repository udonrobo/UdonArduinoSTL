...
namespace std
{
	...
	EA_CONSTEXPR bool cmp_equal(const T x, const U y) EA_NOEXCEPT
	{
		...
		static_assert(std::is_integral<T>::value && !std::is_same<std::remove_cv_t<T>, bool>::value && !std::is_same<std::remove_cv_t<T>, char>::value);
		static_assert(std::is_integral<U>::value && !std::is_same<std::remove_cv_t<U>, bool>::value && !std::is_same<std::remove_cv_t<U>, char>::value);
	...
	EA_CONSTEXPR bool in_range(const U x) EA_NOEXCEPT
	{
		...
		return std::cmp_greater_equal(x, std::numeric_limits<T>::min()) && std::cmp_less_equal(x, std::numeric_limits<T>::max());
	}
	#endif
	...
	struct pair
	{
		...
		template <typename TT1 = T1,
		          typename TT2 = T2,
		          class = std::enable_if_t<std::is_default_constructible<TT1>::value &&
		                                     std::is_default_constructible<TT2>::value>>
		EA_CONSTEXPR pair()
		    : first(), second()
		{
		}
		...
		template <typename TT1 = T1, typename TT2 = T2, typename = std::enable_if_t<std::is_default_constructible<TT2>::value>>
		...
		// error: with 'std::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
			...
			template <typename TT2 = T2, typename = std::enable_if_t<std::is_default_constructible<TT2>::value>>
	...
	// https://bugs.llvm.org/show_bug.cgi?id=38374
		...
		template<typename T>
		using single_pair_ctor_sfinae = std::enable_if_t<std::is_default_constructible<T>::value>;
	#else
		template<typename>
		...
		template <
		    typename TT1 = T1,
		    typename TT2 = T2,
		    class = std::enable_if_t<std::is_copy_constructible<TT1>::value && std::is_copy_constructible<TT2>::value>>
		...
		template <
		    typename U,
		    typename V,
		    class = std::enable_if_t<std::is_convertible<const U&, T1>::value && std::is_convertible<const V&, T2>::value>>
		...
		template <typename U,
		          typename V,
		          typename = std::enable_if_t<std::is_convertible<U, T1>::value && std::is_convertible<V, T2>::value>>
		EA_CPP14_CONSTEXPR pair(U&& u, V&& v)
		    : first(std::forward<U>(u)), second(std::forward<V>(v))
		{
		}

		template <typename U, typename = std::enable_if_t<std::is_convertible<U, T1>::value>>
		EA_CPP14_CONSTEXPR pair(U&& x, const T2& y)
			: first(std::forward<U>(x)), second(y)
		{
		}

		template <typename V, typename = std::enable_if_t<std::is_convertible<V, T2>::value>>
		...
		template <class... Args1,
		          class... Args2,
		          typename = std::enable_if_t<std::is_constructible<first_type, Args1&&...>::value &&
	...
	public:
		pair& operator=(const pair& p)
		    EA_NOEXCEPT_IF(std::is_nothrow_copy_assignable<T1>::value&& std::is_nothrow_copy_assignable<T2>::value)
		{
			first = p.first;
			second = p.second;
			return *this;
		...
		pair& operator=(pair&& p)
		    EA_NOEXCEPT_IF(std::is_nothrow_move_assignable<T1>::value&& std::is_nothrow_move_assignable<T2>::value)
		{
		...
		void swap(pair& p) EA_NOEXCEPT_IF(std::is_nothrow_swappable<T1>::value&& std::is_nothrow_swappable<T2>::value)
		{
...
///////////////////////////////////////////////////////////////
// C++17 structured bindings support for std::pair
//
	...
	namespace std
	{
		...
		template <class... Ts>
		class tuple_size<::std::pair<Ts...>> : public ::std::integral_constant<size_t, sizeof...(Ts)>
		{
		};

		template <size_t I, class... Ts>
		class tuple_element<I, ::std::pair<Ts...>> : public ::std::tuple_element<I, ::std::pair<Ts...>>
		{
		};
