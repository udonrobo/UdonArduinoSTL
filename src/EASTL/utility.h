...
namespace eastl
{
	...
	EA_CONSTEXPR bool cmp_equal(const T x, const U y) EA_NOEXCEPT
	{
		...
		static_assert(eastl::is_integral<T>::value && !eastl::is_same<eastl::remove_cv_t<T>, bool>::value && !eastl::is_same<eastl::remove_cv_t<T>, char>::value);
		static_assert(eastl::is_integral<U>::value && !eastl::is_same<eastl::remove_cv_t<U>, bool>::value && !eastl::is_same<eastl::remove_cv_t<U>, char>::value);

		using UT = eastl::make_unsigned_t<T>;
		using UU = eastl::make_unsigned_t<U>;

		if constexpr (eastl::is_signed<T>::value == eastl::is_signed<U>::value)
		{
			return x == y;
		}
		else if (eastl::is_signed<T>::value)
		{
	...
	struct pair
	{
		...
		template <typename TT1 = T1,
		          typename TT2 = T2,
		          class = eastl::enable_if_t<eastl::is_default_constructible<TT1>::value &&
		                                     eastl::is_default_constructible<TT2>::value>>
		EA_CONSTEXPR pair()
		    : first(), second()
		{
		}
		...
		template <typename TT1 = T1, typename TT2 = T2, typename = eastl::enable_if_t<eastl::is_default_constructible<TT2>::value>>
		...
		// error: with 'eastl::pair<T1, T2>::pair(const T1&) [with T1 = const int&; T2 = const int&]'
			...
			template <typename TT2 = T2, typename = eastl::enable_if_t<eastl::is_default_constructible<TT2>::value>>
	...
	// https://bugs.llvm.org/show_bug.cgi?id=38374
		...
		template<typename T>
		using single_pair_ctor_sfinae = eastl::enable_if_t<eastl::is_default_constructible<T>::value>;
	#else
		template<typename>
		...
		template <
		    typename TT1 = T1,
		    typename TT2 = T2,
		    class = eastl::enable_if_t<eastl::is_copy_constructible<TT1>::value && eastl::is_copy_constructible<TT2>::value>>
		...
		template <
		    typename U,
		    typename V,
		    class = eastl::enable_if_t<eastl::is_convertible<const U&, T1>::value && eastl::is_convertible<const V&, T2>::value>>
		...
		template <typename U,
		          typename V,
		          typename = eastl::enable_if_t<eastl::is_convertible<U, T1>::value && eastl::is_convertible<V, T2>::value>>
		EA_CPP14_CONSTEXPR pair(U&& u, V&& v)
		    : first(eastl::forward<U>(u)), second(eastl::forward<V>(v))
		{
		}

		template <typename U, typename = eastl::enable_if_t<eastl::is_convertible<U, T1>::value>>
		EA_CPP14_CONSTEXPR pair(U&& x, const T2& y)
			: first(eastl::forward<U>(x)), second(y)
		{
		}

		template <typename V, typename = eastl::enable_if_t<eastl::is_convertible<V, T2>::value>>
		...
		template <class... Args1,
		          class... Args2,
		          typename = eastl::enable_if_t<eastl::is_constructible<first_type, Args1&&...>::value &&
		                                        eastl::is_constructible<second_type, Args2&&...>::value>>
	...
	public:
		pair& operator=(const pair& p)
		    EA_NOEXCEPT_IF(eastl::is_nothrow_copy_assignable<T1>::value&& eastl::is_nothrow_copy_assignable<T2>::value)
		{
			first = p.first;
			second = p.second;
			return *this;
		...
		pair& operator=(pair&& p)
		    EA_NOEXCEPT_IF(eastl::is_nothrow_move_assignable<T1>::value&& eastl::is_nothrow_move_assignable<T2>::value)
		{
