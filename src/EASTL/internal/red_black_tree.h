...
// Specify /EHsc 4571 - catch(...) semantics changed since Visual C++ 7.1;
...
#define EASTL_RBTREE_DEFAULT_NAME \
    EASTL_DEFAULT_NAME_PREFIX " rbtree"    // Unless the user overrides something,
                                           // this is "EASTL rbtree".
#endif
...
#define EASTL_RBTREE_LEGACY_SWAP_BEHAVIOUR_REQUIRES_COPY_CTOR 0
    ...
    public:
        ...
        char       mColor;    // We only need one bit here, would be nice if we could stuff
                              // that bit somewhere else.
    };
    ...
    {
        Value mValue;    // For set and multiset, this is the user's value, for map and
                         // multimap, this is a pair of key/value.

// This type is never constructed, so to avoid a MSVC warning we "delete" the
// copy constructor.
//
// Potentially we could provide a constructor that would satisfy the compiler
// and change the code to use this constructor instead of constructing mValue in
// place within an unconstructed rbtree_node.
#if defined(_MSC_VER)
    ...
    inline EASTL_API rbtree_node_base*
    ...
    {
        if ((pNode->mpNodeParent->mpNodeParent == pNode) &&
    ...
    ///
    inline EASTL_API rbtree_node_base* RBTreeRotateLeft(rbtree_node_base* pNode, rbtree_node_base* pNodeRoot)
    {
    ...
    /// discuss the topic in detail.
    ///
    inline EASTL_API rbtree_node_base* RBTreeRotateRight(rbtree_node_base* pNode, rbtree_node_base* pNodeRoot)
    {
    ...
    inline EASTL_API void RBTreeInsert(rbtree_node_base* pNode,
                                       ...
                                       RBTreeSide        insertionSide)
    {
        ...
        {
            pNodeParent->mpNodeLeft = pNode;    // Also makes (leftmost = pNode) when
                                                // (pNodeParent == pNodeAnchor)
        ...
        else
        {
            ...
            if (pNodeParent == pNodeAnchor->mpNodeRight)
                pNodeAnchor->mpNodeRight =
                    pNode;    // Maintain rightmost pointing to max node
        }

        // Rebalance the tree.
        while ((pNode != pNodeRootRef) &&
               (pNode->mpNodeParent->mColor == kRBTreeColorRed))
        {
            EA_ANALYSIS_ASSUME(pNode->mpNodeParent != NULL);
            rbtree_node_base* const pNodeParentParent =
                pNode->mpNodeParent->mpNodeParent;
    ...
    struct rbtree_iterator
    {
        ...
        typedef eastl_size_t                           size_type;    // See config.h for the definition of
                                                                     // eastl_size_t, which defaults to size_t.
    ...
    // operator[] (which only map and set have and multimap and multiset don't
    // have).
    //
    ///////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        rb_base(const Compare& compare)
            : rb_base_compare_ebo<Compare>(compare)
        {
        }
    };
    ...
    /// future if it is deemed worthwhile. Most rbtree implementations don't do
    /// this.
    ///
    ...
    /// the node is just the value. ExtractKey will be either std::use_first (map
    /// and multimap) or std::use_self (set and multiset).
    ///
    ...
    /// in performance improvements but would require a more complicated
    /// implementation.
    ///
    ///////////////////////////////////////////////////////////////////////
    /// find_as
    ...
    class rbtree : public rb_base<Key, Value, Compare, ExtractKey, bUniqueKeys, rbtree<Key, Value, Compare, Allocator, ExtractKey, bMutableIterators, bUniqueKeys>>
    {
    public:
        typedef ptrdiff_t    difference_type;
        typedef eastl_size_t size_type;    // See config.h for the definition of
        ...
        typedef typename type_select<
            bMutableIterators,
            rbtree_iterator<value_type, value_type*, value_type&>,
            rbtree_iterator<value_type, const value_type*, const value_type&>>::type
            iterator;
        ...
        typedef typename type_select<bUniqueKeys, std::pair<iterator, bool>,
                                     iterator>::type
            insert_return_type;    // map/set::insert return a pair,
                                   // multimap/multiset::iterator return an iterator.
        typedef rbtree<Key, Value, Compare, Allocator, ExtractKey, bMutableIterators,
                       bUniqueKeys>
            this_type;
        typedef rb_base<Key, Value, Compare, ExtractKey, bUniqueKeys, this_type>
                                                     base_type;
    ...
    public:
        rbtree_node_base
            mAnchor;         /// This node acts as end() and its mpLeft points to begin(), and
                             /// mpRight points to rbegin() (the last node on the right).
        size_type mnSize;    /// Stores the count of nodes in the tree (not counting the
                             /// anchor node).
        allocator_type
            mAllocator;    // To do: Use base class optimization to make this go away.
    ...
    public:
        ...
        rbtree(const Compare&        compare,
               const allocator_type& allocator = EASTL_RBTREE_DEFAULT_ALLOCATOR);
    ...
    public:
        ...
        // Standard conversion overload to avoid the overhead of mismatched
        // 'pair<const Key, Value>' types.
        template <class P, class = typename std::enable_if<
                               std::is_constructible<value_type, P&&>::value>::type>
        insert_return_type insert(P&& otherValue);

        // Currently limited to value_type instead of P because it collides with
        // insert(InputIterator, InputIterator). To allow this to work with templated
        // P we need to implement a compile-time specialization for the case that P&&
        // is const_iterator and have that specialization handle insert(InputIterator,
        // InputIterator) instead of insert(InputIterator, InputIterator). Curiously,
        // neither libstdc++ nor libc++ implement this function either, which suggests
        // they ran into the same problem I did here and haven't yet resolved it (at
        // least as of March 2014, GCC 4.8.1).
        ...
        // to search. However, there is a potential defect/improvement report on this
        // behaviour: LWG issue #233
        // (http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2005/n1780.html) We
        // follow the same approach as SGI STL/STLPort and use the position as a
        // forced insertion position for the value when possible.
        ...
        void clear();
        void reset_lose_memory();    // This is a unilateral reset to an initially empty
                                     // state. No destructors are called, no deallocation
                                     // occurs.

        iterator       find(const key_type& key);
        const_iterator find(const key_type& key) const;

        /// Implements a find whereby the user supplies a comparison of a different
        /// type than the tree's value_type. A useful case of this is one whereby you
        /// have a container of string objects but want to do searches via passing in
        /// char pointers. The problem is that without this kind of find, you need to
        /// do the expensive operation of converting the char pointer to a string so
        /// it can be used as the argument to the find function.
        ///
        /// Example usage (note that the compare uses string as first type and char*
        /// as second):
        ///     set<string> strings;
    ...
    protected:
        ...
        template <class... Args>
        iterator DoInsertValueImpl(node_type* pNodeParent, bool bForceToLeft,
                                   const key_type& key, Args&&... args);
        iterator DoInsertValueImpl(node_type* pNodeParent, bool bForceToLeft,
                                   const key_type& key, node_type* pNodeNew);

        std::pair<iterator, bool> DoInsertKey(true_type, const key_type& key);
        iterator                  DoInsertKey(false_type, const key_type& key);

        template <class... Args>
        iterator DoInsertValueHint(true_type, const_iterator position,
                                   Args&&... args);

        template <class... Args>
        iterator DoInsertValueHint(false_type, const_iterator position,
                                   Args&&... args);
        ...
        iterator DoInsertKey(
            true_type, const_iterator position,
            const key_type& key);    // By design we return iterator and not a pair.
        iterator DoInsertKey(false_type, const_iterator position,
                             const key_type& key);
        iterator DoInsertKeyImpl(node_type* pNodeParent, bool bForceToLeft,
                                 const key_type& key);

        node_type* DoGetKeyInsertionPositionUniqueKeys(bool&           canInsert,
                                                       const key_type& key);
        node_type* DoGetKeyInsertionPositionNonuniqueKeys(const key_type& key);

        node_type* DoGetKeyInsertionPositionUniqueKeysHint(const_iterator  position,
                                                           bool&           bForceToLeft,
                                                           const key_type& key);
        node_type* DoGetKeyInsertionPositionNonuniqueKeysHint(const_iterator  position,
                                                              bool&           bForceToLeft,
                                                              const key_type& key);
    ...
    EASTL_API inline rbtree_node_base*
    RBTreeGetMinChild(const rbtree_node_base* pNodeBase)
    {
    ...
    EASTL_API inline rbtree_node_base*
    RBTreeGetMaxChild(const rbtree_node_base* pNodeBase)
    {
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline rbtree<K, V, C, A, E, bM, bU>::rbtree(const C&              compare,
    ...
    {
        reset_lose_memory();
        swap(x);    // swap will directly or indirectly handle the possibility that
                    // mAllocator != x.mAllocator.
    }
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    template <typename InputIterator>
    inline rbtree<K, V, C, A, E, bM, bU>::rbtree(InputIterator         first,
                                                 InputIterator         last,
                                                 const C&              compare,
                                                 const allocator_type& allocator)
        : base_type(compare)
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline void
    rbtree<K, V, C, A, E, bM, bU>::set_allocator(const allocator_type& allocator)
    {
        mAllocator = allocator;
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::begin() const EA_NOEXCEPT
    {
        return const_iterator(static_cast<node_type*>(
            const_cast<rbtree_node_base*>(mAnchor.mpNodeLeft)));
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::end() const EA_NOEXCEPT
    {
        return const_iterator(
            static_cast<node_type*>(const_cast<rbtree_node_base*>(&mAnchor)));
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::operator=(const this_type& x)
    {
        if (this != &x)
        {
            ...
            if (x.mAnchor
                    .mpNodeParent)    // mAnchor.mpNodeParent is the rb_tree root node.
            {
                mAnchor.mpNodeParent = DoCopySubtree(
                    (const node_type*)x.mAnchor.mpNodeParent, (node_type*)&mAnchor);
                mAnchor.mpNodeRight = RBTreeGetMaxChild(mAnchor.mpNodeParent);
    ...
    rbtree<K, V, C, A, E, bM, bU>::operator=(this_type&& x)
    {
        if (this != &x)
        {
            clear();    // To consider: Are we really required to clear here? x is going
                        // away soon and will clear itself in its dtor.
            swap(x);    // member swap handles the case that x has a different allocator
                        // than our allocator by doing a copy.
        }
        return *this;
    }
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline typename rbtree<K, V, C, A, E, bM, bU>::this_type&
    rbtree<K, V, C, A, E, bM, bU>::operator=(
        std::initializer_list<value_type> ilist)
    {
        // The simplest means of doing this is to clear and insert. There probably
        // isn't a generic solution that's any more efficient without having prior
        // knowledge of the ilist contents.
        clear();

        for (typename std::initializer_list<value_type>::iterator it    = ilist.begin(),
                                                                  itEnd = ilist.end();
             it != itEnd; ++it)
    ...
    void rbtree<K, V, C, A, E, bM, bU>::swap(this_type& x)
    {
...
#endif
        {
            ...
            // dynamically allocated, we can't do a swap of it but must do a more
            // elaborate procedure. This is the downside to having the mAnchor be like
            // this, but otherwise we consider it a good idea to avoid allocating memory
            // for a nominal container instance.

            // We optimize for the expected most common case: both pointers being
            // non-null.
            if (mAnchor.mpNodeParent &&
                x.mAnchor.mpNodeParent)    // If both pointers are non-null...
            {
                ...
                // We need to fix up the anchors to point to themselves (we can't just
                // swap them).
            ...
            else if (mAnchor.mpNodeParent)
            {
                ...
                // We need to fix up our anchor to point it itself (we can't have it swap
                // with x).
            ...
            else if (x.mAnchor.mpNodeParent)
            {
                ...
                // We need to fix up x's anchor to point it itself (we can't have it swap
                // with us).
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    template <class... Args>
    inline typename rbtree<K, V, C, A, E, bM, bU>::
        insert_return_type    // map/set::insert return a pair,
                              // multimap/multiset::iterator return an iterator.
    ...
    rbtree<K, V, C, A, E, bM, bU>::emplace_hint(const_iterator position,
                                                Args&&... args)
    {
        return DoInsertValueHint(has_unique_keys_type(), position,
    ...
    {
        // Need to use forward instead of move because P&& is a "universal reference"
        // instead of an rvalue reference.
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::insert(const_iterator position,
                                          value_type&&   value)
    {
        return DoInsertValueHint(has_unique_keys_type(), position, std::move(value));
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const key_type& k, M&& obj)
    {
        ...
        if (iter == end())
        {
            return insert(value_type(piecewise_construct, std::forward_as_tuple(k),
    ...
    rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(key_type&& k, M&& obj)
    {
        ...
        if (iter == end())
        {
            return insert(value_type(piecewise_construct,
    ...
    template <class M>
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::insert_or_assign(const_iterator  hint,
                                                    const key_type& k, M&& obj)
    {
        ...
        {
            return insert(hint,
                          value_type(piecewise_construct, std::forward_as_tuple(k),
                                     std::forward_as_tuple(std::forward<M>(obj))));
        }
        else
        {
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeys(
        bool& canInsert, const key_type& key)
    {
        ...
        node_type* pCurrent =
            (node_type*)mAnchor.mpNodeParent;    // Start with the root node.
        node_type* pLowerBound =
            (node_type*)&mAnchor;    // Set it to the container end for now.
        node_type* pParent;          // This will be where we insert the new node.

        bool bValueLessThanNode = true;    // If the tree is empty, this will result in
                                           // an insertion at the front.
        ...
        while (EASTL_LIKELY(pCurrent))    // Do a walk down the tree.
        {
            ...
            if (bValueLessThanNode)
            {
                EASTL_VALIDATE_COMPARE(
                    !compare(extractKey(pCurrent->mValue),
                             key));    // Validate that the compare function is sane.
        ...
        pParent = pLowerBound;    // pLowerBound is actually upper bound right now (i.e.
                                  // it is > value instead of <=), but we will make it
                                  // the lower bound below.

        if (bValueLessThanNode)    // If we ended up on the left side of the last parent
                                   // node...
        {
            if (EASTL_LIKELY(
                    pLowerBound !=
                    (node_type*)mAnchor
                        .mpNodeLeft))    // If the tree was empty or if we otherwise need
                                         // to insert at the very front of the tree...
            {
        ...
        // Since here we require values to be unique, we will do nothing if the value
        // already exists.
        if (compare(
                extractKey(pLowerBound->mValue),
                key))    // If the node is < the value (i.e. if value is >= the node)...
        {
            EASTL_VALIDATE_COMPARE(!compare(
                key, extractKey(pLowerBound->mValue)));    // Validate that the compare
                                                           // function is sane.
            canInsert = true;
            return pParent;
        }

        // The item already exists (as found by the compare directly above), so return
        // false.
        canInsert = false;
        return pLowerBound;
    }
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::node_type*
    rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionNonuniqueKeys(
        const key_type& key)
    {
        // This is the pathway for insertion of non-unique keys (multimap and
        // multiset, but not map and set).
        node_type* pCurrent =
            (node_type*)mAnchor.mpNodeParent;    // Start with the root node.
        node_type* pRangeEnd =
            (node_type*)&mAnchor;    // Set it to the container end for now.
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(true_type, value_type&& value)
    {
        ...
        if (canInsert)
        {
            const iterator itResult(
                DoInsertValueImpl(pPosition, false, key, std::move(value)));
    ...
    template <class... Args>
    std::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool>
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(
        true_type, Args&&... args)    // true_type means keys are unique.
    {
        // This is the pathway for insertion of unique keys (map and set, but not
        // multimap and multiset). Note that we return a pair and not an iterator.
        // This is because the C++ standard for map and set is to return a pair and
        // not just an iterator.

        node_type* pNodeNew = DoCreateNode(
            std::forward<Args>(args)...);    // Note that pNodeNew->mpLeft, mpRight,
                                             // mpParent, will be uninitialized.
    ...
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValue(
        false_type, Args&&... args)    // false_type means keys are not unique.
    {
        // We have a problem here if sizeof(value_type) is too big for the stack. We
        // may want to consider having a specialization for large value_types. To do:
        // Change this so that we call DoCreateNode(std::forward<Args>(args)...) here
        // and use the value from the resulting pNode to get the key, and make
        // DoInsertValueImpl take that node as an argument. That way there is no value
    ...
    template <class... Args>
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueImpl(node_type*      pNodeParent,
                                                     bool            bForceToLeft,
                                                     const key_type& key,
                                                     Args&&... args)
    {
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueImpl(node_type*      pNodeParent,
                                                     ...
                                                     node_type*      pNodeNew)
    {
        EASTL_ASSERT_MSG(pNodeNew != nullptr,
                         "node to insert to the rbtree must not be null");

        RBTreeSide  side;
        extract_key extractKey;

        // The reason we may want to have bForceToLeft == true is that
        // pNodeParent->mValue and value may be equal. In that case it doesn't matter
        // what side we insert on, except that the C++ LWG #233 improvement report
        // suggests that we should use the insert hint position to force an ordering.
        // So that's what we do.
        if (bForceToLeft || (pNodeParent == &mAnchor) ||
            compare(key, extractKey(pNodeParent->mValue)))
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    std::pair<typename rbtree<K, V, C, A, E, bM, bU>::iterator, bool>
    rbtree<K, V, C, A, E, bM, bU>::DoInsertKey(
        true_type, const key_type& key)    // true_type means keys are unique.
    {
        // This is the pathway for insertion of unique keys (map and set, but not
        // multimap and multiset). Note that we return a pair and not an iterator.
        // This is because the C++ standard for map and set is to return a pair and
    ...
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertKey(
        false_type, const key_type& key)    // false_type means keys are not unique.
    {
        node_type* pPosition = DoGetKeyInsertionPositionNonuniqueKeys(key);
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::node_type*
    rbtree<K, V, C, A, E, bM, bU>::DoGetKeyInsertionPositionUniqueKeysHint(
        const_iterator position, bool& bForceToLeft, const key_type& key)
    {
        extract_key extractKey;

        if ((position.mpNode != mAnchor.mpNodeRight) &&
            (position.mpNode !=
             &mAnchor))    // If the user specified a specific insertion position...
        {
            iterator itNext(position.mpNode);
            ++itNext;

            // To consider: Change this so that 'position' specifies the position after
            // where the insertion goes and not the position before where the insertion
            // goes. Doing so would make this more in line with user expectations and
            // with LWG #233.
            const bool bPositionLessThanValue =
                compare(extractKey(position.mpNode->mValue), key);

            if (bPositionLessThanValue)    // If (value > *position)...
            {
                EASTL_VALIDATE_COMPARE(!compare(
                    key,
                    extractKey(position.mpNode->mValue)));    // Validate that the compare
                                                              // function is sane.

                const bool bValueLessThanNext =
                    compare(key, extractKey(itNext.mpNode->mValue));

                if (bValueLessThanNext)    // If value < *itNext...
                {
                    EASTL_VALIDATE_COMPARE(
                        !compare(extractKey(itNext.mpNode->mValue),
                                 key));    // Validate that the compare function is sane.

                    if (position.mpNode->mpNodeRight)
                    {
                        bForceToLeft = true;    // Specifically insert in front of (to the left
                                                // of) itNext (and thus after 'position').
            ...
            bForceToLeft = false;
            return NULL;    // The above specified hint was not useful, then we do a
                            // regular insertion.
        }

        if (mnSize &&
            compare(extractKey(((node_type*)mAnchor.mpNodeRight)->mValue), key))
        {
            EASTL_VALIDATE_COMPARE(!compare(
                key, extractKey(
                         ((node_type*)mAnchor.mpNodeRight)
                             ->mValue)));    // Validate that the compare function is sane.
            bForceToLeft = false;
        ...
        const_iterator position, bool& bForceToLeft, const key_type& key)
    {
             ...
             &mAnchor))    // If the user specified a specific insertion position...
        {
            ...
            if (!compare(
                    key,
                    extractKey(position.mpNode->mValue)) &&    // If value >= *position &&
                !compare(extractKey(itNext.mpNode->mValue),
                         key))    // if value <= *itNext...
            {
                if (position.mpNode
                        ->mpNodeRight)    // If there are any nodes to the right... [this
                                          // expression will always be true as long as we
                                          // aren't at the end()]
                {
                    bForceToLeft = true;    // Specifically insert in front of (to the left of)
                                            // itNext (and thus after 'position').
                    return itNext.mpNode;
                }
        ...
        // This pathway shouldn't be commonly executed, as the user shouldn't be
        // calling this hinted version of insert if the user isn't providing a useful
        // hint.
        if (mnSize &&
            !compare(key, extractKey(((node_type*)mAnchor.mpNodeRight)
                                         ->mValue)))    // If we are non-empty and the
                                                        // value is >= the last node...
        {
            bForceToLeft = false;
            return (node_type*)mAnchor.mpNodeRight;
        }
    ...
    template <class... Args>
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
        true_type, const_iterator position,
        Args&&... args)    // true_type means keys are unique.
    {
        // This is the pathway for insertion of unique keys (map and set, but not
        ...
        bool       bForceToLeft;
        node_type* pPosition =
            DoGetKeyInsertionPositionUniqueKeysHint(position, bForceToLeft, key);
    ...
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
        false_type, const_iterator position,
        Args&&... args)    // false_type means keys are not unique.
    {
        // This is the pathway for insertion of non-unique keys (multimap and
        // multiset, but not map and set).
        //
        ...
        bool       bForceToLeft;
        node_type* pPosition =
            DoGetKeyInsertionPositionNonuniqueKeysHint(position, bForceToLeft, key);
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
        true_type, const_iterator position,
        value_type&& value)    // true_type means keys are unique.
    {
        // This is the pathway for insertion of unique keys (map and set, but not
        // multimap and multiset).
        //
        ...
        if (pPosition)
            return DoInsertValueImpl(pPosition, bForceToLeft, key, std::move(value));
        else
            return DoInsertValue(has_unique_keys_type(), std::move(value)).first;
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertValueHint(
        false_type, const_iterator position,
        value_type&& value)    // false_type means keys are not unique.
    {
        // This is the pathway for insertion of non-unique keys (multimap and
        // multiset, but not map and set).
        //
        ...
        if (pPosition)
            ...
            return DoInsertValue(has_unique_keys_type(), std::move(value));
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertKey(
        true_type, const_iterator position,
        const key_type& key)    // true_type means keys are unique.
    {
        bool       bForceToLeft;
        node_type* pPosition =
            DoGetKeyInsertionPositionUniqueKeysHint(position, bForceToLeft, key);
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoInsertKey(
        false_type, const_iterator position,
        const key_type& key)    // false_type means keys are not unique.
    {
        // This is the pathway for insertion of non-unique keys (multimap and
        // multiset, but not map and set).
        //
        ...
        if (pPosition)
            ...
            return DoInsertKey(has_unique_keys_type(),
                               key);    // We are empty or we are inserting at the end.
    }

    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::DoInsertKeyImpl(node_type*      pNodeParent,
                                                   bool            bForceToLeft,
                                                   const key_type& key)
    {
        RBTreeSide  side;
        ...
        node_type* const pNodeNew =
            DoCreateNodeFromKey(key);    // Note that pNodeNew->mpLeft, mpRight,
                                         // mpParent, will be uninitialized.
        RBTreeInsert(pNodeNew, pNodeParent, &mAnchor, side);
        mnSize++;
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    void rbtree<K, V, C, A, E, bM, bU>::insert(
        std::initializer_list<value_type> ilist)
    {
        for (typename std::initializer_list<value_type>::iterator it    = ilist.begin(),
                                                                  itEnd = ilist.end();
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    template <typename InputIterator>
    void rbtree<K, V, C, A, E, bM, bU>::insert(InputIterator first,
                                               InputIterator last)
    {
        for (; first != last; ++first)
            DoInsertValue(has_unique_keys_type(),
                          *first);    // Or maybe we should call 'insert(end(), *first)'
                                      // instead. If the first-last range was sorted then
    ...
    inline void rbtree<K, V, C, A, E, bM, bU>::reset_lose_memory()
    {
        ...
        // unilaterally resets the container to an empty state without freeing the
        // memory of the contained objects. This is useful for very quickly tearing
        // down a container built into scratch memory.
        mAnchor.mpNodeRight  = &mAnchor;
        mAnchor.mpNodeLeft   = &mAnchor;
        mAnchor.mpNodeParent = NULL;
    ...
    rbtree<K, V, C, A, E, bM, bU>::erase(const_iterator position)
    {
        ...
        --mnSize;    // Interleave this between the two references to itNext. We expect
                     // no exceptions to occur during the code below.
        ++position;
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::iterator
    rbtree<K, V, C, A, E, bM, bU>::erase(const_iterator first,
                                         const_iterator last)
    {
        // We expect that if the user means to clear the container, they will call
        // clear.
        if (EASTL_LIKELY(
                (first.mpNode != mAnchor.mpNodeLeft) ||
                (last.mpNode != &mAnchor)))    // If (first != begin or last != end) ...
        {
    ...
    rbtree<K, V, C, A, E, bM, bU>::erase(const_reverse_iterator first,
                                         const_reverse_iterator last)
    {
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline void rbtree<K, V, C, A, E, bM, bU>::erase(const key_type* first,
                                                     const key_type* last)
    {
        // We have no choice but to run a loop like this, as the first/last range
        // could have values that are discontiguously located in the tree. And some
        // may not even be in the tree.
        while (first != last)
            erase(*first++);
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::find(const key_type& key)
    {
        ...
        //    return ((it.mpNode == &mAnchor) || compare(key,
        //    extractKey(it.mpNode->mValue))) ? iterator(&mAnchor) : it;
        // We don't currently implement the above because in practice people tend to
        // call find a lot with trees, but very uncommonly call lower_bound.
        extract_key extractKey;
        ...
        while (EASTL_LIKELY(pCurrent))    // Do a walk down the tree.
        {
            if (EASTL_LIKELY(!compare(extractKey(pCurrent->mValue),
                                      key)))    // If pCurrent is >= key...
            {
                pRangeEnd = pCurrent;
                pCurrent  = (node_type*)pCurrent->mpNodeLeft;
            }
            ...
            {
                EASTL_VALIDATE_COMPARE(!compare(
                    key, extractKey(pCurrent->mValue)));    // Validate that the compare
                                                            // function is sane.
                pCurrent = (node_type*)pCurrent->mpNodeRight;
            }
        }

        if (EASTL_LIKELY((pRangeEnd != &mAnchor) &&
                         !compare(key, extractKey(pRangeEnd->mValue))))
    ...
    rbtree<K, V, C, A, E, bM, bU>::find_as(const U& u, Compare2 compare2)
    {
        ...
        while (EASTL_LIKELY(pCurrent))    // Do a walk down the tree.
        {
            if (EASTL_LIKELY(!compare2(extractKey(pCurrent->mValue),
                                       u)))    // If pCurrent is >= u...
            {
                pRangeEnd = pCurrent;
                pCurrent  = (node_type*)pCurrent->mpNodeLeft;
            ...
            {
                EASTL_VALIDATE_COMPARE(!compare2(
                    u, extractKey(pCurrent->mValue)));    // Validate that the compare
                                                          // function is sane.
                pCurrent = (node_type*)pCurrent->mpNodeRight;
            }
        }
        ...
        if (EASTL_LIKELY((pRangeEnd != &mAnchor) &&
                         !compare2(u, extractKey(pRangeEnd->mValue))))
            return iterator(pRangeEnd);
        return iterator((node_type*)&mAnchor);
    }
    ...
    rbtree<K, V, C, A, E, bM, bU>::upper_bound(const key_type& key)
    {
        ...
        while (EASTL_LIKELY(pCurrent))    // Do a walk down the tree.
        {
            if (EASTL_LIKELY(compare(
                    key, extractKey(pCurrent->mValue))))    // If key is < pCurrent...
            {
                EASTL_VALIDATE_COMPARE(
                    !compare(extractKey(pCurrent->mValue),
                             key));    // Validate that the compare function is sane.
    ...
    // To do: Move this validate function entirely to a template-less
    // implementation.
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    bool rbtree<K, V, C, A, E, bM, bU>::validate() const
    {
        // Red-black trees have the following canonical properties which we validate
        // here:
        //   1 Every node is either red or black.
        //   2 Every leaf (NULL) is black by defintion. Any number of black nodes may
        //   appear in a sequence. 3 If a node is red, then both its children are
        //   black. Thus, on any path from
        //     the root to a leaf, red nodes must not be adjacent.
        //   4 Every simple path from a node to a descendant leaf contains the same
        //   number of black nodes. 5 The mnSize member of the tree must equal the
        //   number of nodes in the tree. 6 The tree is sorted as per a conventional
        //   binary tree. 7 The comparison function is sane; it obeys strict weak
        //   ordering. If compare(a,b) is true, then compare(b,a) must be false. Both
        //   cannot be true.
        ...
        if (mnSize)
        {
            ...
            const size_t nBlackCount =
                RBTreeGetBlackCount(mAnchor.mpNodeParent, mAnchor.mpNodeLeft);
            ...
            for (const_iterator it = begin(); it != end(); ++it, ++nIteratedSize)
            {
                ...
                // Verify #7 above.
                if (pNodeRight &&
                    compare(extractKey(pNodeRight->mValue), extractKey(pNode->mValue)) &&
                    compare(extractKey(pNode->mValue),
                            extractKey(pNodeRight->mValue)))    // Validate that the compare
                                                                // function is sane.
                    return false;

                // Verify #7 above.
                if (pNodeLeft &&
                    compare(extractKey(pNodeLeft->mValue), extractKey(pNode->mValue)) &&
                    compare(extractKey(pNode->mValue),
                            extractKey(pNodeLeft->mValue)))    // Validate that the compare
                                                               // function is sane.
                    return false;
                ...
                if (pNodeLeft &&
                    compare(extractKey(pNode->mValue), extractKey(pNodeLeft->mValue)))
                    return false;

                if (!pNodeRight &&
                    !pNodeLeft)    // If we are at a bottom node of the tree...
                {
                    // Verify item #4 above.
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    inline int
    rbtree<K, V, C, A, E, bM, bU>::validate_iterator(const_iterator i) const
    {
        // To do: Come up with a more efficient mechanism of doing this.

        for (const_iterator temp = begin(), tempEnd = end(); temp != tempEnd;
             ++temp)
        {
            if (temp == i)
    ...
    {
        auto* pNode = (node_type*)allocate_memory(mAllocator, sizeof(node_type),
                                                  EASTL_ALIGN_OF(node_type), 0);
        EASTL_ASSERT_MSG(
            pNode != nullptr,
            "the behaviour of std::allocators that return nullptr is not defined.");
    ...
    {
        // Note that this function intentionally leaves the node pointers
        // uninitialized. The caller would otherwise just turn right around and modify
        // them, so there's no point in us initializing them to anything (except in a
        // debug build).
    ...
    rbtree<K, V, C, A, E, bM, bU>::DoCreateNode(Args&&... args)
    {
        ...
        {
#endif
            ::new (std::addressof(pNode->mValue))
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::node_type*
    rbtree<K, V, C, A, E, bM, bU>::DoCreateNode(const node_type* pNodeSource,
    ...
    template <typename K, typename V, typename C, typename A, typename E, bool bM,
              bool bU>
    typename rbtree<K, V, C, A, E, bM, bU>::node_type*
    rbtree<K, V, C, A, E, bM, bU>::DoCopySubtree(const node_type* pNodeSource,
                                                 node_type*       pNodeDest)
    {
        ...
        try
        {
            ...
            if (pNodeSource->mpNodeRight)
                pNewNodeRoot->mpNodeRight = DoCopySubtree(
                    (const node_type*)pNodeSource->mpNodeRight, pNewNodeRoot);

            node_type* pNewNodeLeft;

            for (pNodeSource              = (node_type*)pNodeSource->mpNodeLeft,
                pNodeDest                 = pNewNodeRoot;
                 pNodeSource; pNodeSource = (node_type*)pNodeSource->mpNodeLeft,
                pNodeDest                 = pNewNodeLeft)
            {
                ...
                // Copy the right side of the tree recursively.
                if (pNodeSource->mpNodeRight)
                    pNewNodeLeft->mpNodeRight = DoCopySubtree(
    ...
    template <typename K, typename V, typename A, typename C, typename E, bool bM,
              bool bU>
    inline bool operator==(const rbtree<K, V, C, A, E, bM, bU>& a,
                           const rbtree<K, V, C, A, E, bM, bU>& b)
    {
        return (a.size() == b.size()) && std::equal(a.begin(), a.end(), b.begin());
    }

    // Note that in operator< we do comparisons based on the tree value_type with
    // operator<() of the value_type instead of the tree's Compare function. For
    // set/multiset, the value_type is T, while for map/multimap the value_type is a
    // pair<Key, T>. operator< for pair can be seen by looking utility.h, but it
    // basically is uses the operator< for pair.first and pair.second. The C++
    // standard appears to require this behaviour, whether intentionally or not. If
    // anything, a good reason to do this is for consistency. A map and a vector
    // that contain the same items should compare the same.
    template <typename K, typename V, typename A, typename C, typename E, bool bM,
              bool bU>
    inline bool operator<(const rbtree<K, V, C, A, E, bM, bU>& a,
                          const rbtree<K, V, C, A, E, bM, bU>& b)
    ...
    template <typename K, typename V, typename A, typename C, typename E, bool bM,
              bool bU>
    inline void swap(rbtree<K, V, C, A, E, bM, bU>& a,
                     rbtree<K, V, C, A, E, bM, bU>& b)
    {
        a.swap(b);
    }
