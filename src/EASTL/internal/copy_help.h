...
namespace std
{
	...
	namespace internal {
		...
		using is_contiguous_iterator_helper = std::is_same<IC, EASTL_ITC_NS::contiguous_iterator_tag>;
	#else
		template <typename IC>
		using is_contiguous_iterator_helper = std::false_type;
	#endif

		template <typename InputIterator, typename OutputIterator>
		struct can_be_memmoved_helper {
			using IIC = typename std::iterator_traits<InputIterator>::iterator_category;
			using OIC = typename std::iterator_traits<OutputIterator>::iterator_category;
			using value_type_input = typename std::iterator_traits<InputIterator>::value_type;
			using value_type_output = typename std::iterator_traits<OutputIterator>::value_type;

			static constexpr bool value = std::is_trivially_copyable<value_type_output>::value &&
				                          std::is_same<value_type_input, value_type_output>::value &&
				                         (std::is_pointer<InputIterator>::value  || is_contiguous_iterator_helper<IIC>::value) &&
				                         (std::is_pointer<OutputIterator>::value || is_contiguous_iterator_helper<OIC>::value);
	...
	{
		return OutputIterator(std::move_and_copy_chooser<isMove>(std::unwrap_iterator(first), std::unwrap_iterator(last), std::unwrap_iterator(result))); // Have to convert to OutputIterator because unwrap_iterator(result) could be a T*
	}
