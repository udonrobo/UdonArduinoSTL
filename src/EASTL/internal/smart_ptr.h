...
namespace std
{
	...
	namespace Internal
	{
		...
		template <typename P1, typename P2, bool = std::is_same<std::remove_cv_t<typename pointer_traits<P1>::element_type>,
																    std::remove_cv_t<typename pointer_traits<P2>::element_type>>::value>
	...
	struct default_delete
	{
		...
		{
			static_assert(std::internal::is_complete_type<T>::value, "Attempting to call the destructor of an incomplete type");
			delete p;
		}
	};
