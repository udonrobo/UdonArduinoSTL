...
namespace std
{
	...
	{
		template <class T>
		using detect_pointer_traits_to_address = decltype(std::pointer_traits<T>::to_address(std::declval<const T&>()));

		template <class T>
		using result_detect_pointer_traits_to_address = std::is_detected<detect_pointer_traits_to_address, T>;
	}

	template<class T>
	EA_CPP14_CONSTEXPR T* to_address(T* p) noexcept
	{
		static_assert(!std::is_function<T>::value, "Cannot call to_address with a function pointer. C++20 20.2.4.1 - Pointer conversion.");
		return p;
	}

	template <class Ptr, typename std::enable_if<Internal::result_detect_pointer_traits_to_address<Ptr>::value, int>::type = 0>
	EA_CPP14_CONSTEXPR auto to_address(const Ptr& ptr) noexcept -> decltype(std::pointer_traits<Ptr>::to_address(ptr))
	{
