...
// 4244 - Argument: conversion from 'int' to 'const std::vector<T>::value_type', possible loss of data
...
namespace std
{
	...
	{
		static_assert(std::has_equality<T>::value, "T must be comparable");
		...
		else // else (size == capacity)
		{
			...
			pointer const   pNewData  = DoAllocate(nNewSize);
				...
				{   // To do: We are not handling exceptions properly below.  In particular we don't want to
					// call std::destruct on the entire range if only the first part of the range was constructed.
				...
				catch(...)
				{
					if(pNewEnd)
						...
						std::destruct(pNewData + nPosSize);                                       // The exception occurred during the first uninitialized move, so destroy only the value at nPosSize.
					DoFree(pNewData, nNewSize);
					throw;
				}
			#else
				...
				pNewEnd = std::uninitialized_move_ptr_if_noexcept(destPosition, mpEnd, ++pNewEnd);            // Question: with exceptions disabled, do we assume all operations are noexcept and thus there's no need for uninitialized_move_ptr_if_noexcept?
			#endif
